...
from agent.service import get_agent_session, run_agent_input, run_agent_prompt
...
@router.post("/agent/input")
async def run_agent_input_route(
    payload: dict,
    gemini_api_key: str | None = Header(
        default=None,
        alias="X-Wolfie-Gemini-Api-Key",
    ...
    if not isinstance(message, str) or not message.strip():
        raise HTTPException(status_code=400, detail="A message string is required")
...
async def get_default_agent_state():
    return get_agent_session("default")


@router.get("/agent/state/{thread_id}")
async def get_agent_state(thread_id: str):
    return get_agent_session(thread_id)
